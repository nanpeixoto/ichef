	...
	public void adicionarArea() {
		...
		try {
			...
			if (localidades == null || localidades.size() == 0) {
				facesMessager.error(
						"Nenhuma localidade encontrada, verifique se a �rea informada n�o est� vinculada a outro entregador e/ou se �rea possui localidades vinculadas");
				return;
			}

			if (!itemAdicionado) {
				facesMessager.error("Localidade(s) n�o vinculada(s) verifique duplicidade(s)");
			}
