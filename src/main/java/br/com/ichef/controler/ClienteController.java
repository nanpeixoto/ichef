...
public class ClienteController extends BaseController {
	...
	public String Salvar() throws Exception {
		...
		FacesUtil.addInfoMessage("Novo cliente criado c�digo: " + getEntity().getId());
	...
	public void adicionarTelefone() {
		if (!getTelefone().equals("5571 ")) {
			...
			if (!existeTelefonePrincipal(null)) {
				...
				facesMessager.error("J� existe um telefone principal para esse cliente");
			}
			// } else {
			// facesMessager.error("Telefone j� adicionado para essa cliente");
			// }
		}
	}
	...
	public void onTabChange(TabChangeEvent event) {
		try {
			...
			e.printStackTrace();
			facesMessager.error(
					"N�o foi possivel efeturar a opera��o, entre em contato com administrador do sistema e informe o erro #0001");
		}
	}
		...
		if (getLocalidade() != null) {
			if (!existeLocalidade(getLocalidade())) {
				if (!existeEnderecoPrincipal(null)) {
					...
					facesMessager.error("J� existe um endereco principal para esse cliente");
				}
			} else {
				facesMessager.error("Endereco j� adicionado para essa cliente");
			}
		} else {
	...
	public String validarCliente() throws Exception {
		...
		if (telefone.length() < 8) {
			FacesUtil.addErroMessage("Informe um telefone v�lido para continuar");
			return "";
		}
	...
	public void editarLinhaTelefone(RowEditEvent event) throws Exception {
		...
		if (telefoneEditado.isTelefonePrincipal()) {
			if (!existeTelefonePrincipal(telefoneEditado)) {
				...
				FacesUtil.addErroMessage("J� existe um telefone principal para esse cliente");
			}
		}
	...
	public void excluirCarteira(ClienteCarteira obj) {
		try {
			...
			e.printStackTrace();
			facesMessager.error(
					"N�o foi possivel efeturar a opera��o, entre em contato com administrador do sistema e informe o erro #0002");
		}
	}
