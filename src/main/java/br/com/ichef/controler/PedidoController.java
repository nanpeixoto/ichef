		...
		try {
			...
			if (camposObrigatoriosPedido()) {
				...
				try {
					...
					//System.out.println("ERRO: adicionarPedido - findValorDebito - Debito n�o encontrado");
				}
				//configuracao
				Configuracao config = (Configuracao) JSFUtil.getSessionMapValue("configuracao");
				
				
				// SE FOR AT� 10 ABRIR POUPUP
			...
			if (camposObrigatoriosPedido()) {
				...
				if (getEntity().getEntregador() == null) {
					...
					if (getEntity().getEntregador().getQuantiadadeQuentinha() != null
						...
						facesMessager.error("Quantidade m�xima do Entregador j� atingida, qtd dispon�vel: "
				...
				if ((quantidadeJaPedida - getEntity().getQuantidade()) <= 5) {
					FacesUtil.addInfoMessage(
							"Quantdade dispon�vel para o prato:" + (quantidadeJaPedida - getEntity().getQuantidade()));
				}
